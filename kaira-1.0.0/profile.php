...
$recordCountActive    = 0;
$recordCountEnded     = 0;
$recordCountPending   = 0;
$recordCountApproved  = 0;
$recordCountRejected  = 0;
$recordCountConfirmed = 0;
...
foreach ($activityRecords as $r) {
    ...
    if ($r['submission_status'] === 'approved' && !$r['confirmed']) {
        $recordCountApproved++;
    }
...
<head>
  ...
  <style>
    ...
    * {
    ...
    .category-box.open .category-title::after {
    ...
    .activity-card:hover {
      box-shadow: 0 4px 20px rgba(45,58,74,.12);
    }
    ...
    .tag.today {
      background: var(--tag-today);
      color: var(--tag-today-text);
    }

    .tag.upcoming {
      background: var(--tag-up);
      color: var(--tag-up-text);
    }

    .tag.expired {
      background: var(--tag-exp);
      color: var(--tag-exp-text);
    }
        ...
        <div class="record-list">
          ...
          <?php foreach ($activityRecords as $record): ?>
            <?php
              ...
              if (!empty($record['event_end'])) {
                  $eventEnded = strtotime($record['event_end']) < $nowTime;
              } elseif (!empty($record['event_start'])) {
                  $eventEnded = strtotime($record['event_start']) < $nowTime;
              } else {
                  $eventEnded = false;
              }
            ...
            <article class="record-card"
              ...
              <div class="record-meta">
                ...
                <?php if (!empty($record['reviewed_at'])): ?>
                  ・審核時間：<?= htmlspecialchars($record['reviewed_at']) ?>
                <?php endif; ?>
              </div>
