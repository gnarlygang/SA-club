...
try {
    ...
    // 取得活動資料（驗證為本人發佈）
    $stmt = $pdo->prepare("SELECT * FROM activities WHERE id = :id AND user_id = :uid LIMIT 1");
    $stmt->execute([
        ":id"  => $activity_id,
        ":uid" => $_SESSION["user_id"] ?? null,
    ]);
...
$f = [
    ...
    "event_start"     => $_POST["event_start"]     ?? date("Y-m-d\TH:i", strtotime($activity["event_start"])),
    "event_end"       => $_POST["event_end"]       ?? ($activity["event_end"] ? date("Y-m-d\TH:i", strtotime($activity["event_end"])) : ""),
    "location"        => $_POST["location"]        ?? $activity["location"],
    "organizer"       => $_POST["organizer"]       ?? $activity["organizer"],
    "fee"             => $_POST["fee"]             ?? $activity["fee"],
    "target"          => $_POST["target"]          ?? $activity["target"],
    "signup_deadline" => $_POST["signup_deadline"] ?? $activity["signup_deadline"],
];
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .edit-card-body { padding: 40px 48px 48px; }
    ...
    @media (max-width: 600px) {
      ...
      .edit-card-body { padding: 28px 24px 36px; }
      .edit-card-header { padding: 28px 24px 20px; }
    }
  </style>
</head>
<body>
...
<div class="edit-wrapper">
  <div class="edit-card">
    ...
    <div class="edit-card-body">
      ...
      <form method="POST" action="activity_edit.php?id=<?= $activity_id ?>" autocomplete="off">
        ...
        <div class="mb-4">
          ...
          <div class="hint-text">主辦單位依發佈帳號自動帶入，不可修改。</div>
        </div>
      ...
      <a href="activity_list.php" class="btn-cancel">
        <i class="bi bi-arrow-left me-1"></i>取消，返回已發佈活動
      </a>
