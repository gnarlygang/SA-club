...
if ($_SERVER["REQUEST_METHOD"] === "POST" && $is_logged_in) {
    ...
    if ($action === 'delete_post') {
        ...
        if (!in_array($item_type, ['post','comment'])) {
            ...
            $chk = $pdo->prepare("SELECT id FROM forum_reports WHERE item_type=? AND item_id=? AND reporter_id=? AND status='pending'");
            $chk->execute([$item_type, $item_id, $current_user_id]);
            if ($chk->fetch()) {
                $error = "你已檢舉過此內容，請等待管理員處理。";
            } else {
...
try {
    $stmt = $pdo->prepare("
        ...
        WHERE fp.id=:id AND fp.is_deleted=0 LIMIT 1
    ");
...
foreach ($all_comments as $c) {
    if ($c['parent_id']) $replies_map[$c['parent_id']][] = $c;
    else                 $top_comments[] = $c;
}
...
<body>
<style>
:root{--sh:0 2px 12px rgba(60,80,120,.08);--bd:#e8ecf0;--inp:#c8d0dc;--err:#c0392b}
body{font-family:"Microsoft JhengHei",sans-serif;background:#f0f2f5;min-height:100vh;display:flex;flex-direction:column}
.wrap{max-width:860px;margin:0 auto;width:100%;padding:32px 16px 60px}
...
.post-card{background:#fff;border-radius:16px;box-shadow:var(--sh);padding:36px 40px;margin-bottom:24px}
...
.post-meta{display:flex;align-items:center;gap:16px;font-size:13px;color:#9aa;margin-bottom:28px;padding-bottom:20px;border-bottom:1px solid var(--bd);flex-wrap:wrap}
...
.cmt-card{background:#fff;border-radius:12px;box-shadow:var(--sh);padding:18px 22px}
.cmt-card.highlight{box-shadow:0 0 0 2px #6e8ab0,var(--sh)}
...
.cmt-form-card{background:#fff;border-radius:14px;box-shadow:var(--sh);padding:26px 30px;margin-top:22px}
...
.login-prompt{background:#f4f6f9;border:1px solid var(--bd);border-radius:10px;padding:16px 20px;font-size:14px;color:#667;text-align:center;margin-top:24px}
...
function setReply(parentId, authorName) {
    ...
    if (repliesList && !repliesList.classList.contains('open')) {
        const toggleBtn = repliesList.previousElementSibling;
        if (toggleBtn && toggleBtn.classList.contains('replies-toggle'))
            toggleReplies(toggleBtn, parentId);
    }
